		...
		for(int i=0;i<w.getY();i++) {
			for(int j=0;j<w.getX();j++) {
				...
				for(int i2=0;i2<w.tab_Volcan.size();i2++) {
					if(w.tab_Volcan.get(i2).liquide[j][i]>=1) { //on verifie si il y a de la lave
						if(i-1>=0 && w.tab_Volcan.get(i2).liquide[j][i-1]==0 && (i+1<w.getY() && w.tab_Volcan.get(i2).liquide[j][i+1]>=7)) { //haut
