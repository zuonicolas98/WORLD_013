		...
		for(int i=x-2; i<=x+2; i++) { //on parcourt les cases voisines du lapin avec un rayon de 2 cases (voisinage de Moore)
			for(int j=y-2; j<=y+2; j++) {
				if( (i<0) || (j<0) || (i>=w.getX()) && (j>=w.getY())) { //si on sort du tableau, on passe a l'iteration suivante
					...
					for(int k=0; k<w.tab_Animal.size(); k++) { //parcours de la liste d'animaux
						if( (w.tab_Animal.get(k).getX() == i) && (w.tab_Animal.get(k).getY() == j) && (w.tab_Animal.get(k) instanceof Chevre) || ((i>=0) && (j>=0) && (i<w.getX()) && (j<w.getY()) && w.liquide[i][j]>0)) { //on trouve un predateur dans un rayon de 2 cases
							...
							if(i<x) {     // Si le predateur se trouve a gauche par rapport au lapin...
								...
								if(j<y && y+1<w.getY() && w.getWorld()[x][y+1] != 3 && direction == -1 && w.getWorld()[x][y+1] != 2 && (w.n.alti[x][y+1]!=-1)) {		// et si le predateur se trouve en haut du lapin
									direction = 2;
								}
								else if(j>y && y-1>=0 && w.getWorld()[x][y-1] != 3 && direction == -1 && w.getWorld()[x][y-1] != 2 && (w.n.alti[x][y-1]!=-1)) {	// ou si le predateur se trouve en bas du lapin
									direction = 0;
								}
							}
