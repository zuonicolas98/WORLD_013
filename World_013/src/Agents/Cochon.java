...
public class Cochon extends Animal{
	...
	public void step() { //bouge selon l'environnement	
		...
		//reproduction
		if(Math.random()<0.9 && nb_cochon<35) {
			for(int k=0; k < w.tab_Animal.size(); k++) {
	...
	public boolean fuir() { //fuit in predateur dans son environnement
		action=1;
		for(int i=x-2; i<=x+2; i++) { //on parcourt les cases voisines du cochon avec un rayon de 2 cases (voisinage de Moore)
			for(int j=y-2; j<=y+2; j++) {
				if( (i<0) || (j<0) || (i>=w.getX()) && (j>=w.getY())) { //si on sort du tableau, on passe a l'iteration suivante
					...
					for(int k=0; k<w.tab_Animal.size(); k++) { //parcours de la liste d'animaux
						if( (w.tab_Animal.get(k).getX() == i) && (w.tab_Animal.get(k).getY() == j) && (w.tab_Animal.get(k) instanceof Lapin) || ( (i>=0) && (j>=0) && (i<w.getX()) && (j<w.getY()) && w.liquide[i][j]>0) ) { //on trouve un predateur dans un rayon de 2 cases
							...
							//initialisation direction
							if(i<x) {     // Si le predateur se trouve a gauche par rapport au cochon...
								if(x+1<w.getX() && w.getWorld()[x+1][y] != 3 && w.getWorld()[x+1][y] != 2 && (w.n.alti[x+1][y]!=-1)) {
									direction = 1;
								}else
									direction = -1;
								
								if(j<y && y+1<w.getY() && w.getWorld()[x][y+1] != 3 && direction == -1 && w.getWorld()[x][y+1] != 2 && (w.n.alti[x][y+1]!=-1)) {		// et si le predateur se trouve en haut du cochon.
									direction = 2;
								}
								else if(j>y && y-1>=0 && w.getWorld()[x][y-1] != 3 && direction == -1 && w.getWorld()[x][y-1] != 2 && (w.n.alti[x][y-1]!=-1)) {	// ou si le predateur se trouve en bas du cochon
									direction = 0;
								}
							}
							
							else if (i==x) { //Si le predateur se trouve sur le meme x que le cochon...
							...
							else {			//Si le predateur se trouve à droite du cochon...
