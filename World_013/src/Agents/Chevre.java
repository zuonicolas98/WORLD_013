			...
			for(int j=y-2; j<=y+2; j++) {
				if( (i<0) || (j<0) || (i>=w.getX()) && (j>=w.getY())) { //si on sort du tableau, on passe a l'iteration suivante
					...
					for(int k=0; k<w.tab_Animal.size(); k++) { //parcours de la liste d'animaux
						if( (unique==0) && (w.tab_Animal.get(k).getX() == i) && (w.tab_Animal.get(k).getY() == j) && (w.tab_Animal.get(k) instanceof Cochon)) { //on trouve une proie dans un rayon de 2 cases
							b=true;
							unique=1;
							if(i<x) {     // Si la proie se trouve a gauche par rapport à la chevre...
								x--;
								direction = 3;
								if(j<y) {		// et si la proie se trouve en haut de la chevre.
									y--;
									direction = 0;
								}
								else if(j>y) {	// et si la proie se trouve en bas de la chevre.
									y++;
									direction = 2;
								}
							}
